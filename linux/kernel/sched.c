...
// 该宏取信号 nr 在信号位图中对应位的二进制数值。信号编号 1-32。比如信号 5 的位图数值等于
// 1<<(5-1) = 16 = 00010000b。
#define _S(nr) (1 << ((nr)-1))
// 除了 SIGKILL 和 SIGSTOP 信号以外其他信号都是可阻塞的(…1011,1111,1110,1111,1111b)。
...
// 时钟中断处理程序（kernel/system_call.s，176）
extern int timer_interrupt(void);
// 系统调用中断处理程序（kernel/system_call.s，80）
...
// jiffies 进行优化处理，也不要挪动位置，并且需要从内存中取其值。因为时钟中断处理过程等程序会修改它的值。
...
// 下面结构用于设置堆栈 ss:esp（数据段选择符，指针），见 head.s，第 23 行。
...
void schedule(void)
{
    ...
    for (p = &LAST_TASK; p > &FIRST_TASK; --p)
        if (*p)
        {
            ...
            // 如果设置过任务的定时值 alarm，并且已经过期(alarm<jiffies),则在信号位图中置 SIGALRM信号，
            // 即向任务发送 SIGALARM 信号。然后清 alarm。该信号的默认操作是终止进程。
            // jiffies是系统从开机开始算起的滴答数（10ms/滴答）。定义在 sched.h 第 139 行。
    ...
    while (1)
    {
        ...
        // 比较每个就绪状态任务的 counter（任务运行时间的递减滴答计数）值，哪一个值大，运行时间还不长，
        // next 就指向哪个的任务号。
        while (--i)
        {
            if (!*--p)
        ...
        // 如果比较得出有 counter 值不等于 0 的结果，或者系统中没有一个可运行的任务存在（此时 c仍然为-1，next=0），
        // 则退出当前的循环，执行 switch_to 任务切换操作。否则就根据每个任务的优先权值，
        // 更新每一个任务的 counter 值，然后回到最上面的while循环重新比较。
        // counter 值的计算方式为 counter = counter /2 + priority。注意，这里计算过程不考虑进程的状态。
        if (c)
            break;
        for (p = &LAST_TASK; p > &FIRST_TASK; --p)
            if (*p)
    ...
    // 切换到任务号为 next 的任务，并运行之。
    // 用下面宏（定义在 sched.h 中）把当前任务指针 current 指向任务号为 next 的任务，并切换
    // 到该任务中运行。 next 被初始化为 0。因此若系统中没有任何其他任务可运行时，
    // 则 next 始终为 0。因此调度函数会在系统空闲时去执行任务 0。 此时任务 0 仅执行 pause()系统调用，并又会调用本函数。
    switch_to(next);
}
...
// 函数参数 p 是等待任务队列头指针。指针是含有一个变量地址的变量。这里参数 p 使用了指针的指针形式 '**p'，
// 这是因为 C 函数参数只能传值，没有直接的方式让被调用函数改变调用该函数程序中变量的值。
// 但是指针'*p'指向的目标（这里是任务结构）会改变，因此为了能修改调用该函数程序中原来就是指针变量的值，
// 就需要传递指针'*p'的指针，即'**p'。参见程序前示例图中p 指针的使用情况。
...
repeat:
    ...
    // 执行到这里，说明本任务真正被唤醒执行。此时等待队列头指针应该指向本任务，若它为空，
    // 则表明调度有问题，于是显示警告信息。最后我们让头指针指向在我们前面进入队列 的任务（*p = tmp）。
    // 若确实存在这样一个任务，即队列中还有任务（tmp 不为空），
    // 就唤醒之。最先进入队列的任务在唤醒后运行时最终会把等待队列头指针置成 NULL。
    if (!*p)
...
// 将当前任务置为可中断的等待状态（TASK_INTERRUPTIBLE），并放入头指针*p 指定的等待队列中。
...
void do_timer(long cpl)
{
    ...
    current->counter = 0;
    if (!cpl)
        return; // 对于内核态程序，不依赖 counter 值进行调度。
    schedule();
}
...
void sched_init(void)
{
    ...
    // Linux 系统开发之初，内核不成熟。内核代码会被经常修改。Linus 怕自己无意中修改了这些
    ...
    // FIRST_TSS_ENTRY 和 FIRST_LDT_ENTRY 的值分别是 4 和 5，定义在 include/linux/sched.h中；
    // gdt 是一个描述符表数组（include/linux/head.h ），实际上对应程序 head.s 中
    // 第 234 行上的全局描述符表基址（_gdt）。因此 gdt + FIRST_TSS_ENTRY 即为
    // gdt[FIRST_TSS_ENTRY]（即是 gdt[4]），也即 gdt 数组第 4 项的地址。 参见include/asm/system.h,第 65 行开始。

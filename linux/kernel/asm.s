/*
 * asm.s 程序中包括大部分的硬件故障（或出错）处理的底层次代码。页异常由内存管理程序mm 处理，所以不在这里。
 * 此程序还处理（希望是这样）由于 TS-位而造成的 fpu 异常，因为fpu 必须正确地进行保存/恢复处理，这些还没有测试过。
 */
# TS - Task Switched, 控制寄存器 CR0 的位 3。请参见 4.1.3 节。
...
# 不了一个合法除操作的结果时，也会产生这个异常。标号'_do_divide_error'实际上是
# C 语言函数 do_divide_error()编译后所生成模块中对应的名称。该函数在 traps.c 中,101 行。
_divide_error:
	pushl $_do_divide_error     # 首先把将要调用的函数地址入栈
no_error_code:                  # 这里是无出错号处理的入口处，见下面jmp no_error_code
	...
	push %es
	push %fs
	pushl $0		            # 将数值 0 作为出错码入栈
	lea 44(%esp),%edx           # 取有效地址，即栈中原调用返回地址处的栈指针位置,并压入堆栈
	pushl %edx                  # 初始化段寄存器 ds、es 和 fs，加载内核数据段选择符
	movl $0x10,%edx
	mov %dx,%ds
	mov %dx,%es
	mov %dx,%fs
    # 下行的'*'号表示调用操作数指定地址处的函数，称为间接调用。这句的含义是调用引起本次异常
    # 的 C 处理函数，例如 do_divide_error(esp, error_code)等。
    # 在函数返回后，addl $8,%esp会将堆栈指针加 8 相当于执行两次 pop 操作，弹出（丢弃）最后入栈的两个 C 函数参数（33 行和 35 行入栈 的值）,
    # 让堆栈指针重新指向寄存器 fs 入栈处。
...
# 以下中断在调用时 CPU 会在中断返回地址之后将出错号压入堆栈，因此返回时也需要将出错号弹出（参见图 5.3(b)）。
...
# int0x20 -- 时钟中断（_timer_interrupt）在 kernel/sys_call.s，189 行。
# int0x80 -- 系统调用（_system_call）在 kernel/sys_call.s，84 行。
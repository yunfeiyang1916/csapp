/*
* 本文件是文件控制选项头文件。其中主要定义了文件控制函数 fcntl()和文件创建或打开函数中用到的一些选项。
*/
#ifndef _FCNTL_H
#define _FCNTL_H
...
#define O_CREAT 00100       // 如果文件不存在就创建。fcntl 函数不用。
...
#define F_GETFD 1 // 取句柄标志。仅 1 个标志 FD_CLOEXEC。
...
#define F_GETLK 5  // 返回阻止锁定的 flock 结构。

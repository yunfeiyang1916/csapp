/*
 * fs.h 头文件中定义了有关文件系统的一些常数和结构。主要包含高速缓冲区中缓冲块的数据结构、
 * MINIX 1.0 文件系统中超级块和 i 节点结构以及文件表结构和一些管道操作宏。
 */
...
struct d_inode
{
    ...
    // 修改时间（自 1970.1.1:0 算起，秒）
    unsigned long i_time;
    // 组 id(文件拥有者所在的组)
    unsigned char i_gid;
    // 链接数（多少个文件目录项指向该 i 节点）
    unsigned char i_nlinks;
    // 直接(0-6)、间接(7)或双重间接(8)逻辑块号。zone 是区的意思，可译成区段，或逻辑块。
...
struct super_block
{
    ...
    // i 节点位图缓冲块指针数组(占用 8 块，可表示 64M)
    struct buffer_head *s_imap[8];
    // 逻辑块位图缓冲块指针数组（占用 8 块）
    struct buffer_head *s_zmap[8];
    // 超级块所在的设备号
    unsigned short s_dev;
    // 被安装的文件系统根目录的 i 节点。(isup-super i)

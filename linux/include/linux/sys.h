 ...
 * 为什么这不是一个.c 文件？动动脑筋自己想想....
 */
// 0 - 系统启动初始化设置函数。 (kernel/blk_drv/hd.c)
extern int sys_setup();
// 1 - 程序退出。 (kernel/exit.c )
extern int sys_exit();
// 2 - 创建进程。 (kernel/system_call.s)
extern int sys_fork();
...
extern int sys_getpid();       // 20 - 取进程 id。 (kernel/sched.c)
extern int sys_mount();        // 21 - 安装文件系统。 (fs/super.c)
extern int sys_umount();       // 22 - 卸载文件系统。 (fs/super.c)
extern int sys_setuid();       // 23 - 设置进程用户 id。 (kernel/sys.c)
extern int sys_getuid();       // 24 - 取进程用户 id。 (kernel/sched.c)
...
extern int sys_setgid();       // 46 - 设置进程组 id。 (kernel/sys.c)
extern int sys_getgid();       // 47 - 取进程组 id。 (kernel/sched.c)
extern int sys_signal();       // 48 - 信号处理。 (kernel/signal.c)
extern int sys_geteuid();      // 49 - 取进程有效用户 id。 (kenrl/sched.c)
extern int sys_getegid();      // 50 - 取进程有效组 id。 (kenrl/sched.c)
...
extern int sys_setpgid();      // 57 - 设置进程组 id。 (kernel/sys.c)
...
extern int sys_getppid();      // 64 - 取父进程 id。 (kernel/sched.c)
extern int sys_getpgrp();      // 65 - 取进程组 id，等于 getpgid(0)。(kernel/sys.c)
...
extern int sys_setreuid();     // 70 - 设置真实与/或有效用户 id。 (kernel/sys.c)
extern int sys_setregid();     // 71 - 设置真实与/或有效组 id。 (kernel/sys.c)

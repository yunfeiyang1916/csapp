/*
 * 该头文件中以内嵌函数的形式定义了所有字符串操作函数，并且为了提高执行速度使用了内嵌汇编语言。
 * 另外，在开始处还定义了一个 NULL 宏和一个 SIZE_T 类型。
 */
#ifndef _STRING_H_
#define _STRING_H_
...
// 将串 1 看作是包含零个或多个单词(token)的序列，并由分割符字符串 2 中的一个或多个字符
// 分开。第一次调用 strtok()时，将返回指向字符串 1 中第 1 个 token 首字符的指针，并在返
// 回 token 时将一 null 字符写到分割符处。后续使用 null 作为字符串 1 的调用，将用这种方
// 法继续扫描字符串 1，直到没有 token 为止。在不同的调用过程中，分割符串 2 可以不同。

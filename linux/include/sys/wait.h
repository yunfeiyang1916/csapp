/**
 * 该头文件描述了进程等待时的信息。包括一些符号常数和 wait()、waitpid()函数原型声明。
*/
#ifndef _SYS_WAIT_H
#define _SYS_WAIT_H
...
// [ 注：其实 0.12 内核已经支持 WUNTRACED 选项。上面这条注释应该是以前内核版本遗留下来的。 ]
...
// 以下宏定义用于判断 waitpid()函数返回的状态字（第 20、21 行的参数*stat_loc）的含义。
...
#define WIFSIGNALED(s) (((unsigned int)(s)-1 & 0xFFFF) < 0xFF) // 如果由于未捕捉信号而
// 导致子进程退出则为真。

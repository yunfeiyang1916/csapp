...
static int free_ind(int dev, int block)
{
    ...
    // 最后释放设备上的一次间接块。但如果其中有逻辑块没有被释放，则返回 0（失败）。
    if (block_busy)
        return 0;
    else
        return free_block(dev, block); // 成功则返回 1，否则返回 0。
}

...
;----------定义宏--------------------------------------------------------------------
    %macro alloc_core_linear 0                      ;在内核空间中分配虚拟内存，0个参数
...
;-----------代码段-----------------------------------------------------------------------
section code vstart=0x80040000
;----------头部数据段--------------------------------------------------------------------
                                                    ;以下是系统核心的头部，用于加载核心程序 
    core_length dd core_end                         ;[0x00]内核总长度，双字，32位
    core_entry dd start                             ;[0x04]内核代码段入口地址偏移量
...
;-----------通用的异常处理过程--------------------------------------------------------------------
    general_exception_handler:
        mov ebx,excep_msg
        call flat_4gb_code_seg_sel:put_string       ;因为put_string还需要提供给用户程序调用，代码返回使用的是retf，所以这里也需要使用远程调用
...
;-----------加载并重定位用户程序-----------------------------------------------------------------------
    ...
    .b1:
        ...
        mov eax,cr3
        mov cr3,eax                     ;读一次cr3在原样写回去，用于刷新TLB（旁路缓存，会缓存页表）
...
;-----------程序入口-----------------------------------------------------------------------
    start:
        ...
        ;前20个向量是处理器异常使用的
        mov eax,general_exception_handler  ;中断门代码偏移地址
        mov bx,flat_4gb_code_seg_sel       ;中断门代码选择子
        mov cx,0x8e00                      ;32位中断门，0特权级
        call flat_4gb_code_seg_sel:make_gate_descriptor     ;构造中断门
    ...
    .idt0:
        ...
        ;其余为保留或硬件使用的中断向量
        mov eax,general_interrupt_handler  ;中断门代码偏移地址
        mov bx,flat_4gb_code_seg_sel       ;中断门代码选择子
        mov cx,0x8e00                      ;32位中断门，0特权级
        call flat_4gb_code_seg_sel:make_gate_descriptor     ;构造中断门
    ...
    .idt1:
        ...
        mov [ebx+esi*8+4],edx
        inc esi
        cmp esi,255                        ;安装普通的中断处理过程,中断总共有256个
        jle .idt1

        ;设置实时时钟中断处理过程，可以进行任务切换
        mov eax,rtm_0x70_interrupt_handle  ;中断门代码偏移地址
        mov bx,flat_4gb_code_seg_sel       ;中断门代码选择子
        mov cx,0x8e00                      ;32位中断门，0特权级
        call flat_4gb_code_seg_sel:make_gate_descriptor     ;构造中断门

        mov ebx,idt_linear_address         ;中断描述符表的线性地址
        mov [ebx+0x70*8],eax               ;0x70号中断向量是时钟中断
        mov [ebx+0x70*8+4],edx

        ;准备开放中断
        mov word [pidt],256*8-1            ;中断描述符表的界限值
    ...
    .b4:
        ...
        ;初始化程序管理器任务的任务控制块TCB,单个任务块的大小为70字节
